...
describe(`Функция отсчёта времени`, () => {
  let second;
  it(`уменьшает переменную c 10  до нуля`, (done) => {
    second = 10;
    gameplay.countdown(second, (secondInTheEnd) => {
      assert.equal(secondInTheEnd, 0);
      done();
    }, 2);
  });
  it(`уменьшает переменную c 300 до нуля`, (done) => {
    second = 300;
    gameplay.countdown(second, (secondInTheEnd) => {
      assert.equal(secondInTheEnd, 0);
      done();
