...
describe(`Функция проверки таймера`, () => {
  ...
  it(`возвращает 15 (секунд), при таймере в 30 с началом 15 секунд назад`, () => {
    ...
    timer = 30;
    assert.equal(getTimeLeft(startDate, timer, checkDate), 15);
  });
  it(`возвращает 15 (секунд), при таймере в 30 с началом 15 секунд назад
    ...
    timer = 30;
    assert.equal(Math.floor(getTimeLeft(startDate, timer)), 15);
  });
  it(`возвращает -1, если разница дат больше таймера и время вышло`, () => {
    startDate = new Date();
    startDate.setTime(startDate.getTime() + 32000);
    timer = 30;
    assert.equal(Math.floor(getTimeLeft(startDate, timer)), -1);
  });
});
